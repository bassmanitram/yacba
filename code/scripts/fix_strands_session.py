...
def count_messages_from(messages_dir: Path, from_id: int) -> int:
    ...
    return len([
        f for f in messages_dir.glob("message_*.json")
        if int(f.stem.split('_')[1]) >= from_id
    ])


def delete_messages_from(messages_dir: Path, from_id: int, dry_run: bool = False) -> int:
...
def fix_session(session_path: Path, dry_run: bool = False) -> bool:
    ...
    print(f"  {num_to_delete} message(s) will be deleted (message_{orphan_id}.json onwards)")
...
Examples:
  ...
  %(prog)s session_*
    ...
    parser.add_argument(
        'sessions',
        nargs='+',
        help='Session directory/directories to check and fix'
    )
