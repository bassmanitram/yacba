...
class TestConfigUtilsIntegration:
    ...
    def test_discover_and_use_configs(self, tmp_path):
        ...
        mcp_tools.write_text(
            json.dumps(
                {"type": "mcp", "id": "filesystem", "command": "mcp-server-filesystem"}
            )
        )
    ...
    def test_discover_multiple_directories(self, tmp_path):
        ...
        dir1.mkdir()
        (dir1 / "tool1.tools.json").write_text(
            json.dumps({"type": "python", "id": "tool1"})
        )

        dir2 = tmp_path / "tools2"
        dir2.mkdir()
        (dir2 / "tool2.tools.json").write_text(
            json.dumps({"type": "mcp", "id": "tool2"})
        )
