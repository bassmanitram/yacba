...
class TestFilesSpecAction:
    ...
    def test_files_spec_action_not_needed(self):
        ...
        pytest.skip(
            "FilesSpec no longer needed - dataclass-args handles file arguments"
        )
...
class TestArgumentParserIntegration:
    ...
    def test_defaults_are_valid_types(self):
        ...
        for key, value in ARGUMENT_DEFAULTS.items():
            assert isinstance(
                value, valid_types
            ), f"{key} has invalid type {type(value)}"
...
class TestArgumentConfiguration:
    ...
    def test_numeric_defaults_reasonable(self):
        ...
        if (
            "preserve_recent_messages" in ARGUMENT_DEFAULTS
            and "sliding_window_size" in ARGUMENT_DEFAULTS
        ):
            assert (
                ARGUMENT_DEFAULTS["preserve_recent_messages"]
                < ARGUMENT_DEFAULTS["sliding_window_size"]
            )
...
class TestEnvironmentVariables:
    ...
    def test_env_vars_are_valid_types(self):
        ...
        for key, value in ARGUMENTS_FROM_ENV_VARS.items():
            assert isinstance(
                value, valid_types
            ), f"Env var {key} has invalid type {type(value)}"
