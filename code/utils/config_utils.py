...
def discover_tool_configs(
    paths: Union[List[PathLike], PathLike, None],
) -> Tuple[List[str], ToolDiscoveryResult]:
    """
    ...
    for directory in path_list:
        file_paths, failed_configs, files_scanned = _discover_single_directory(
            directory
        )
    ...
    discovery_result = ToolDiscoveryResult(
        [
            {"file_path": path} for path in all_file_paths
...
def _discover_single_directory(
    directory: PathLike,
