...
class ExceptionCategory:
    ...
    PROVIDER_ERROR = "provider"      # AI provider errors (e.g., rate limits, service unavailable)
...
def categorize_exception(exc: Exception) -> str:
    ...
    if any(pattern in exc_module or pattern in exc_type for pattern in provider_patterns):
...
def format_exception(exc: Exception) -> Tuple[str, bool]:
    ...
    force_all_tracebacks = os.environ.get('YACBA_LOG_TRACEBACKS', '').lower() in ('1', 'true', 'yes', 'on')
    ...
    if category == ExceptionCategory.PROVIDER_ERROR:
        ...
        if clean_msg:
            ...
            provider = "Unknown"
            if 'vertex' in exc_type.lower() or 'vertexai' in str(type(exc).__module__).lower():
