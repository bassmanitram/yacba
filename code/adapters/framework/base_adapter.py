...
class DefaultAdapter:
    """
    Adapter for frameworks that support standard system prompts and message
    formats. Handles YACBA's responsibility for framework-specific
    configuration.
    """

    @property
    def expected_exceptions(self) -> Tuple[type[Exception], ...]:
        """Expected exception types that the CLI should catch for this
        framework."""
        return (
    ...
    def adapt_tools(self, tools: List[Tool], model_string: str) -> List[Tool]:
        ...
        For LiteLLM models, cleans the tool schemas to remove
        'additionalProperties', which is not supported by some underlying APIs
        like Google VertexAI.
        ...
        if "litellm" in model_string:
            logger.debug("LiteLLM model detected. Cleaning tool schemas to "
            ...
            for tool in tools:
                if hasattr(tool, 'TOOL_SPEC'):
                    ...
                    module_funcs = [name for name in dir(tool)
                                    if callable(getattr(tool, name))]
                    for func_name in module_funcs:
                        func = getattr(tool, func_name)
                        if hasattr(func, '_tool_spec'):
                            func_name_or_attr = getattr(func, 'name',
                                                        func_name)
                            logger.trace("Cleaning _tool_spec for tool: "
                                         f"{func_name_or_attr}")
                            recursively_remove(func._tool_spec,
    ...
    def prepare_agent_args(
        ...
        Handles YACBA's responsibility for message preparation and system
        prompt handling.
        ...
        Args:
            ...
            emulate_system_prompt: Whether to emulate system prompt in first
                                   user message
        ...
        # Handle system prompt emulation for frameworks that don't support it
        # natively
        if emulate_system_prompt and system_prompt:
            logger.debug("Emulating system prompt by prepending to the first "
                         "user message as requested.")
